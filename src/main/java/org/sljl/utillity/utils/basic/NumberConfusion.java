...
public class NumberConfusion {
	...
	private static final int[][] DIGIT_TABLE = {
			{ 6, 1, 3, 5, 2, 0, 8, 4, 7, 9 }, { 3, 7, 2, 9, 0, 5, 8, 4, 6, 1 },
			{ 7, 1, 5, 8, 9, 2, 0, 6, 3, 4 }, { 3, 2, 8, 7, 1, 0, 5, 6, 4, 9 },
			{ 0, 9, 5, 7, 3, 1, 2, 8, 4, 6 }, { 6, 4, 0, 7, 9, 3, 8, 5, 1, 2 },
			{ 6, 3, 7, 4, 1, 8, 0, 2, 9, 5 }, { 0, 1, 3, 2, 7, 9, 6, 8, 5, 4 },
	 ...
	 * @return
	 */
	public static final long decode(String ciphertext) {
	 ...
	 * @return
	 */
	public static final long decodeWithCompatible(String ciphertext) {
		try {
			return decodeWithException(ciphertext);
		} catch (Exception e) {
	...
	public static final String encode(long id) {
		...
		// 维吉尼亚
		return vigenere.encode("" + footLong, "" + headLong)
				+ LETTER[Math.abs(seed)] + footLong;
	}
	 ...
	 */
	private static long decodeWithException(String ciphertext) {
		// 发现种子字母
		...
		return Long.parseLong(head + "" + foot);
	}
	...
	public static void main(String[] args) {
		long userId = 765432l;
		String aa = NumberConfusion.encode(userId);
		System.out.println(aa);
		System.out.println(NumberConfusion.decode(aa));
	}
